...
#import os
# print(">>> ESTE ES EL BUILDERBOT.PY QUE SE ESTÁ EJECUTANDO <<<")
# print(">>> RUTA REAL:", os.getcwd())
...
def start_SSFarm():
    global Bbot_thread
    if not Bbot_running.is_set():
        Bbot_thread = threading.Thread(target=SSFarmBot, daemon=True)
        Bbot_thread.start()
        log("SSFarm started.")
...
def start_Farm():
    global Bbot_thread
    if not Bbot_running.is_set():
        Bbot_thread = threading.Thread(target=gf.run_farm_cycle, daemon=True)
        Bbot_thread.start()
        log("Farm started.")
...
App.button4 = ctk.CTkButton(App.button_frame, text="Start SSFarm", command=start_SSFarm)
App.button4.grid(row=0, column=2, padx=5)
...
import func as f
f.log = log # Injsect the log function into func.py
